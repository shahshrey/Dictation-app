...
import { DEFAULT_SETTINGS } from '../components/constants';
import { registerGlobalHotkey } from '../components/windowManager';
...
export const setupSettingsHandlers = (
    ...
    try {
      ...
      if (!store) {
        ...
        try {
          ...
          logger.error('Error loading settings from fallback file:', { error: (fileError as Error).message });
        }
      }
      ...
      logger.debug('[DEBUG] Current settings API key length:', { length: settings.apiKey ? settings.apiKey.length : 0 });
  ...
  // Save settings
  ipcMain.handle('save-settings', (_, newSettings: Settings): { success: boolean; error?: string } => {
    try {
      if (store) {
        ...
        try {
          ...
          logger.error('Error saving to electron-store, falling back to file:', { error: (storeError as Error).message });
